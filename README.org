...
#+PROPERTY:  header-args :eval never :padline no :tangle no
...
#+begin_src conf :tangle .sqshrc :noweb yes
# Hey Emacs, this is a -*- conf -*- file
## .sqshrc - Configuration settings and aliases for sqsh.

# Copyright (C) 2010-<<current-year()>> Fabrice Niessen. All rights reserved.
...
NOTE -- It's a common convention to capitalize system schema & metadata tables
for SQL Server.
...
- Inequality:
  ...
  #+begin_src sql
  -- For int (if -32768 can't be a real value).
  ISNULL(a, -32768) <> ISNULL(b, -32768)

  -- For varchar (if '' can't be a real value).
  ISNULL(a, '') <> ISNULL(b, '')
...
** Checking existence (EXISTS) in Oracle trigger

In Oracle, ~EXISTS~ is a SQL construct and cannot be used directly in
PL/SQL.

#+begin_src sql
-- Check if the equipment standard is found in the mapping table.
IF NOT EXISTS (SELECT 1 FROM equipment_standard_mapping WHERE eq_std = :NEW.eq_std) THEN
     -- If not found, set eq_id to the original value and exit.
    :NEW.eq_id := :NEW.eq_id;
    RETURN;
END IF;
#+end_src

Instead, you need to use a ~SELECT ... INTO~ statement to check whether a row
exists.

Here's the equivalent form:
...
SELECT COUNT(*)
INTO v_count
FROM equipment_standard_mapping
WHERE eq_std = :NEW.eq_std;
...
grep -v -- ----- table_rmpct.csv | sed 's_; *_;_g' | sed 's_ *;_;_g' | sponge table_rmpct.csv
#+end_src
...
SELECT *
FROM (
    SELECT TABLESPACE_NAME, FILE_NAME, FILE_ID, BYTES, STATUS, MAXBYTES
    FROM DBA_DATA_FILES
    UNION
    SELECT TABLESPACE_NAME, FILE_NAME, FILE_ID, BYTES, STATUS, MAXBYTES
    FROM DBA_TEMP_FILES
)
WHERE UPPER(TABLESPACE_NAME) LIKE UPPER('%%')
...
Il existe plusieurs façons de vérifier si un interblocage (deadlock) se produit
dans une base de données Oracle. Vous pouvez :

1. *Vérifier le fichier de trace généré par Oracle* : Lorsqu'un interblocage se
   produit, Oracle génère un fichier de trace dans le répertoire UDUMP. Ce
   fichier contient les informations de l'interblocage, y compris les SQL qui
   ont causé l'interblocage.

2. *Utiliser la vue dynamique de performance (V$) d'Oracle* : Oracle fournit
   plusieurs vues dynamiques de performance pour aider à diagnostiquer les
   problèmes de performance, y compris les interblocages.

   Par exemple, vous pouvez utiliser la vue V$LOCK pour voir les verrous
   actuellement détenus dans la base de données, ce qui peut vous aider
   à identifier les interblocages. Vous pouvez exécuter une requête comme
   celle-ci pour voir tous les verrous :

   De plus, la vue V$SESSION_WAIT peut vous aider à voir si des sessions
   attendent des verrous qui sont détenus par d'autres sessions.

3. *Utiliser des outils de surveillance de base de données* : Il existe des outils
   de surveillance de base de données, comme Oracle Enterprise Manager, qui
   peuvent vous aider à identifier et à résoudre les interblocages.

4. *Utiliser la vue DBA_BLOCKERS et DBA_WAITERS* : Ces vues donnent des
   informations sur les sessions qui bloquent et celles qui attendent un verrou.

   Vous pouvez utiliser la commande suivante pour voir les sessions bloqueuses :

   Et cette commande pour voir les sessions en attente :

N'oubliez pas qu'il est important de comprendre la cause sous-jacente de
l'interblocage et de résoudre ce problème, plutôt que de simplement résoudre
l'interblocage lui-même. Dans de nombreux cas, les interblocages sont le
résultat de problèmes de conception de l'application ou de la base de données.
...
#+begin_src sql
SELECT a.sid, a.serial#, a.username, b.sql_text, TO_CHAR(b.last_active_time, 'YYYY-MM-DD HH24:MI:SS') as last_active_time
FROM v$session a
...
Cela devrait vous donner la date et l'heure de la dernière activité pour chaque
commande SQL en cours d'exécution par une session bloqueuse, formatées de la
...
#+begin_src sql
-- Calculate the total number of rows across all user tables (excluding system tables) (for SQL Server).
SELECT SUM(p.row_count) AS total_rows
...
#+begin_src sql
-- Calculate the total number of rows across all user tables (for Oracle).
...
#+begin_src sql
-- List all triggers with detailed information including activation, events,
-- type, status, and full definition.
SELECT
    tbl.name                            AS "TABLE",
...
Display the list of columns in an Oracle table:

#+begin_src sql
-- Retrieve the list of column names from an Oracle table.
